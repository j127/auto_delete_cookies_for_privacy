  ...
  if (pkgVer) {
    if (REGVER.test(pkgVer)) {
      ...
      if (pkgVer === versionTag) {
        ...
        console.error(
          "ERROR:  Version Tag does not match NPM Package Version.  Please revise either version to match and try again."
        );
        process.exitCode = 3;
      }
    } else {
      console.error(
        "ERROR:  NPM Package Version is not in valid semver form.  Please check version in package.json."
      );
      process.exitCode = 2;
    }
  } else {
    console.error(
      "ERROR: Version does not exist in NPM Package.  Please check package.json for a version item."
    );
    process.exitCode = 1;
  }
} else {
